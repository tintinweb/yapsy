...
class PluginManager(object):
    ...
    def __init__(self,
        ...
        #init active_plugins dict
        for k in categories_filter.iterkeys():
            self.ACTIVE_PLUGINS[k]=[]
    ...
    def _getPluginNameAndModuleFromStream(self, infoFileObject, candidate_infofile="<buffered info>"):
        ...
        if not config_parser.has_section("Core"):
            log.warning("Plugin info file has no 'Core' section (in '%s')" % candidate_infofile)
            return (None, None, None)
        if not config_parser.has_option("Core","Name") or not config_parser.has_option("Core","Module"):
            log.warning("Plugin info file has no 'Name' or 'Module' section (in '%s')" % candidate_infofile)
            return (None, None, None)
        # check that the given name is valid
        name = config_parser.get("Core", "Name")
        name = name.strip()
        if PLUGIN_NAME_FORBIDEN_STRING in name:
            log.warning("Plugin name contains forbiden character: %s (in '%s')" % (PLUGIN_NAME_FORBIDEN_STRING,
                                                                                                                                                       candidate_infofile))
            return (None, None, None)
    ...
    def gatherBasicPluginInfo(self, directory,filename):
        ...
        # add plugin description file contents to plugin_info (type __dict__)
        plugin_info.config = config_parser.__dict__['_sections'].copy()
        return plugin_info
...
#               print "%s.loadPlugins" % self.__class__
        ...
        for candidate_infofile, candidate_filepath, plugin_info in self._candidates:
            ...
            try:
                ...
                log.warning("Unable to execute the code in plugin: %s" % candidate_filepath)
                log.warning("\t The following problem occured: %s %s %s %s" % (os.linesep, e,os.linesep,repr(sys.exc_info())))
                if "__init__" in  os.path.basename(candidate_filepath):
...
#               print "%s.collectPlugins" % self.__class__
    ...
    def getPluginByName(self,name,category="Default"):
        ...
        if category in self.category_mapping:
            ...
            #cycle all items and search for name
            retnitems=[]
            for v in self.category_mapping.itervalues():
                for i in v:
                    if i.name == name: retnitems.extend( item)
            if len(retnitems)>0: return retnitems
        return None
    ...
    def activatePluginByName(self,name,category="Default"):
        ...
        if pta_item is not None:
            ...
            if plugin_to_activate is not None:
                ...
                self.ACTIVE_PLUGINS[category].append(pta_item)
    ...
    def deactivatePluginByName(self,name,category="Default"):
        ...
        if category in self.category_mapping:
            ...
            if plugin_to_deactivate is not None:
                ...
                self.ACTIVE_PLUGINS[category].remove(item)
                return plugin_to_deactivate
        return None


    def getActivePlugins(self,category=None,sortCriteria=None,convertHook=None,sortDefault=0,sortHook=None):
        """
        Return a list of active plugins by given category, sortCriteria, conversion hook, defaults and sortHook.

        optionally sort all plugins according to a field in plugin-description-file
            + sortCriteria = lst ['section','key'] of descr.file
            + sortValueType = converts descr. file value (convert from str to int, ..)
            + sortDefault  = default value if descr.file key/section is not present
        """
        retnlst=[]
        if category:
            if self.ACTIVE_PLUGINS.has_key(category):
                retnlst= self.ACTIVE_PLUGINS[category]
        else:
            #return all active plugins if none
            allactive=[]
            for v in self.ACTIVE_PLUGINS.itervalues():
                allactive.extend(v)
            retnlst= allactive

        #do we want to sort (ini sort!)?
        if sortCriteria and isinstance(sortCriteria,list):
            retnlst=self.sortPlugins(retnlst,criteria=sortCriteria,convertHook=convertHook,default=sortDefault,sortHook=sortHook)
        return retnlst

    def sortPlugins(self,pluginlist,criteria,convertHook=None,default=0,sortHook=None):
        '''
                  use like:    pluginlist,   criteria=['Testrun','Priority'], convertHook=valuetype (int), default=defaultval

                we build a list like [ (sortcriteria_val, element), (sortcrit_val, element)   ] and sort val1, highest val last
        '''
        lst = []
        sortHook = sortHook or self.__qsort2d
        convertHook = convertHook or int
        #get all plugins - build list like described above
        for p in pluginlist:
            #get value and store it in format described above
            val = self.__dict_has_keys(p.config,lstkeys=criteria,default=default)
            if isinstance(val,str) and "," in val:
                for val in val.split(","):       #get all values delim'd by  , (multiple priorities = multiple runs)
                    lst.append( (convertHook(val),p)  )
            else:
                lst.append( (convertHook(val),p))
        ...
        lst=sortHook(lst)
        #strip priority info
        lst=[p[1] for p in lst]     #returns v,p
        return lst

    def __qsort2d(self,lstIn):
        """
        Quicksort using lstIn comprehensions
        ...
        if lstIn == []:
            ...
            pivot = lstIn[0]
            lesser = self.__qsort2d([x for x in lstIn[1:] if x < pivot])
            greater = self.__qsort2d([x for x in lstIn[1:] if x >= pivot])
            return lesser + [pivot] + greater


    def __dict_has_keys(self,inputdict,lstkeys,firstrun=True,default=None):
        #
        # checks if a dict hast keys   x[key1][key2][key3]
        #
        if firstrun:
            lstkeys = lstkeys[::]
            lstkeys.reverse()
        k = lstkeys.pop()
        if inputdict.has_key(k):
            if len(lstkeys)<=0:
                return inputdict[k]
            return self.__dict_has_keys(inputdict[k], lstkeys=lstkeys,firstrun=False,default=default)
        return default
